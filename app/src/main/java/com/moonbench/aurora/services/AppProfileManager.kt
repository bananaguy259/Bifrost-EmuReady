...
import com.moonbench.aurora.LedPreset
import com.moonbench.aurora.PresetIcon
...
class AppProfileManager(private val prefs: SharedPreferences) {
    ...
    fun checkForSwitch(context: Context): SwitchResult? {
        ...
        val isAuroraSelf = currentPackage == context.packageName
        val isHome = isHomePackage(context, currentPackage)
        val shouldUseFallback = isAuroraSelf || isHome
        Log.d(TAG, "checkForSwitch: isAuroraSelf=$isAuroraSelf, isHome=$isHome, shouldUseFallback=$shouldUseFallback")
